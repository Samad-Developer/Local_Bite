...
import { z } from "zod"
import { createAddonGroup, updateAddonGroup } from "@/lib/actions/variants/addon-groups"
import { Field, FieldLabel, FieldError, FieldDescription } from "@/components/ui/field"
import { Input } from "@/components/ui/input"
import { Switch } from "@/components/ui/switch"
import { Button } from "@/components/ui/button"
import { AddonGroup } from "@/types/variants.types"
  ...
  function onSubmit(data: FormValues) {
    ...
    startTransition(async () => {
      const result = isEditing
        ? await updateAddonGroup({ id: group.id, ...data })
        : await createAddonGroup({ variantId, ...data })
    ...
    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Controller
          ...
          render={({ field, fieldState }) => (
            <Field data-invalid={fieldState.invalid}>
              <FieldLabel htmlFor="maxSelections">
                Max selections
              </FieldLabel>
              <Input
                id="maxSelections"
